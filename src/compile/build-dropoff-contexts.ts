  ...
  const contexts: DropoffContext[] = [
    {
      ...
      connects_to: ['Gilimanuk Harbor', 'Pemuteran', 'Lovina', 'Ubud', 'Canggu', 'Seminyak', 'Denpasar Airport'],
      required_customer_fields: ['next_destination', 'ferry_or_bali_transfer_preference'],
      cost_impacts: ['ferry_ticket', 'bali_transfer', 'vehicle_crossing_or_handoff'],
      risk_factors: ['ferry_queue', 'late_bali_arrival', 'bali_traffic'],
      source_trace: [{ source: 'manual_seed', ref: 'seed/manual-overrides/pickup-dropoff.yaml', confidence: 'manual_seed' }]
    },
    {
      id: 'surabaya_airport_dropoff',
      label: 'Surabaya Airport dropoff',
      status: 'active',
      confidence: 'manual_seed',
      type: 'airport',
      location_group: 'Surabaya',
      default_buffer_minutes: 180,
      required_customer_fields: ['flight_number', 'departure_time'],
      cost_impacts: ['route_cutoff', 'vehicle_day_usage', 'parking_toll_fuel_allowance'],
      risk_factors: ['city_traffic', 'late_activity_finish', 'airport_buffer_risk'],
      source_trace: [{ source: 'manual_seed', ref: 'seed/manual-overrides/pickup-dropoff.yaml', confidence: 'manual_seed' }]
    },
    {
      id: 'bali_hotel_dropoff',
      label: 'Bali hotel dropoff',
      status: 'active',
      confidence: 'manual_seed',
      type: 'bali_area',
      location_group: 'Bali',
      default_buffer_minutes: 60,
      required_customer_fields: ['hotel_name', 'hotel_area'],
